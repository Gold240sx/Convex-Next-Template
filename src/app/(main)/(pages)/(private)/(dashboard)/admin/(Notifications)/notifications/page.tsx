...
export default function NotificationsPage() {
    ...
    const formSubmissions = useQuery(api.formSubmissions.list)

    if (messages === undefined || formSubmissions === undefined) {
        return (
    ...
    const unreadSubmissions = formSubmissions.filter(s => !s.isRead)
    const totalUnread = unreadMessages.length + unreadSubmissions.length

    // Combine and sort all unread notifications by date
    const allUnread = [
        ...unreadMessages.map(m => ({ type: 'message' as const, data: m, createdAt: m.createdAt })),
        ...unreadSubmissions.map(s => ({ type: 'submission' as const, data: s, createdAt: s.createdAt }))
    ].sort((a, b) => b.createdAt - a.createdAt)

    // Combine and sort read notifications
    const allRead = [
        ...messages.filter(m => m.readAt).map(m => ({ type: 'message' as const, data: m, createdAt: m.readAt! })),
        ...formSubmissions.filter(s => s.isRead).map(s => ({ type: 'submission' as const, data: s, createdAt: s.createdAt }))
            ...
            <div className="space-y-6">
                {totalUnread === 0 ? (
                    ...
                    <div className="grid gap-4">
                        {allUnread.map((notification, idx) => {
                            if (notification.type === 'message') {
                                ...
                                return (
                                    <Link 
                                        ...
                                        href={`/admin/form-submissions/${submission._id}`}
                                        className="group relative bg-card border border-border hover:border-purple-500/40 transition-all duration-300 rounded-2xl overflow-hidden flex flex-col md:flex-row items-start md:items-center p-6 gap-6 shadow-sm hover:shadow-md"
                                    >
                                        <div className="w-14 h-14 shrink-0 bg-purple-500/10 rounded-2xl flex items-center justify-center border border-purple-500/20 group-hover:bg-purple-500/20 transition-colors">
                                            <FileText className="w-7 h-7 text-purple-600 dark:text-purple-400" />
                                        </div>
                                        
                                        <div className="flex-1 space-y-1">
                                            <div className="flex items-center gap-3">
                                                <h3 className="text-lg font-bold text-foreground uppercase italic tracking-tight group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                                                    New Form Submission
                                                </h3>
                                                <span className="px-2 py-0.5 bg-purple-500/10 border border-purple-500/20 rounded-md text-[9px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest">
                                                    FORM
                                                </span>
                                            </div>
                                            <p className="text-muted-foreground text-sm line-clamp-1">
                                                <span className="font-bold text-foreground/80">{submission.formName}</span> submitted {submission.submitterName && `by ${submission.submitterName}`}
                                            </p>
                                            <div className="flex items-center gap-4 text-[11px] font-bold text-muted-foreground/60 uppercase tracking-wider pt-1">
                                        ...
                                        <div className="flex items-center gap-3 self-end md:self-center">
                                            <span className="text-xs font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest group-hover:mr-2 transition-all">VIEW SUBMISSION</span>
                                            <ChevronRight className="w-5 h-5 text-zinc-300 dark:text-zinc-700 group-hover:text-purple-500 transition-colors" />
                                        </div>
                                    </Link>
                                )
                            }
            ...
            {allRead.length > 0 && (
                <div className="space-y-6 pt-10">
                    ...
                    <div className="grid gap-4 opacity-60 hover:opacity-100 transition-opacity">
                        {allRead.map((notification, idx) => {
                            if (notification.type === 'message') {
                                const message = notification.data
                                return (
                                    <Link 
                                        key={`read-msg-${message._id}`}
                                        href={`/admin/user-messages/${message._id}`}
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                            <h4 className="text-sm font-bold text-foreground">{submission.formName} Submission</h4>
                                            <p className="text-xs text-muted-foreground italic">Viewed on {new Date(submission.createdAt).toLocaleDateString()}</p>
                                        </div>
                                        <ExternalLink className="w-4 h-4 text-zinc-300" />
                                    </Link>
                                )
